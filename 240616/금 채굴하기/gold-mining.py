...
def get_num_of_golds(row, col, k):
    return sum([board[i][j] for i in range(N) for j in range(N) if abs(row - i) + abs(col - j) <= k])

max_golds = 0
for row in range(N):
    for col in range(N):
        for side in range(2 * (N - 1) + 1):
            golds = get_num_of_golds(row, col, side)

            if golds * M >= get_area(side):
